...
public class FirstPersonCamera : CameraController
{
    ...
    /// <summary>
    /// Optional: Use this if your player uses a Rigidbody for movement.
    /// If not set, the camera will try to get velocity from the MovementController (if it implements IVelocity).
    /// </summary>
    [Property, Category( "Components" )]
	...
	private Vector3 GetVelocity()
	{
		...
		// Fall back to movement controller
		if ( Movement.IsValid() )
		{
    ...
    protected override void UpdateCameraPosition()
    {
       ...
       var expectedVertical = velocity.z * Time.Delta;
       
       // If there's a sudden vertical position change (teleport, respawn, etc.)
       // snap the camera instead of smoothing
